	...
	public static void onTick(TickEvent.ClientTickEvent event) {
		...
		if (KeyRecorder.keyActivateParCoolState.isPressed()) {
			...
			if (stamina.isExhausted()) {
				player.sendStatusMessage(new TextComponentTranslation(TranslateKeys.WARNING_ACTIVATION_EXHAUSTED), false);
				return;
			}
			...
			player.sendStatusMessage(new TextComponentTranslation(active ? TranslateKeys.MESSAGE_ACTIVATION_ACTIVE : TranslateKeys.MESSAGE_ACTIVATION_INACTIVE), false);
			ParCoolConfig.sync();
		}
	}
